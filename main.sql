...
CREATE TABLE t02_slot (
    ...
    CONSTRAINT CK_T02_UNQ UNIQUE(C02_SLOT_DATE, C02_SLOT_START_TIME, C02_SLOT_END_TIME, C02_ROOM_ID),
...
CREATE TABLE t19_private_notifications (
     ...
     C19_SEND_TO INT NOT NULL,   -- NG/NG/NG/NG
     C19_CENTER_ID INT NOT NULL, -- có thể getId của manager tạo thông báo từ đây
     CONSTRAINT FK_T19_USER FOREIGN KEY (C19_SEND_TO) REFERENCES t14_user(C14_USER_ID),
     CONSTRAINT FK_T19_CENTER FOREIGN KEY (C19_CENTER_ID) REFERENCES t03_center(C03_CENTER_ID)
);
...
CREATE TABLE t20_public_notifications (
     ...
     C20_CENTER_ID INT NOT NULL, -- có thể getId của manager tạo thông báo từ đây
     CONSTRAINT FK_T20_CENTER FOREIGN KEY (C20_CENTER_ID) REFERENCES t03_center(C03_CENTER_ID)
);
...
CREATE TABLE t21_center_posts (
     ...
     C21_CENTER_ID INT NOT NULL, -- có thể getId của manager tạo thông báo từ đây
     CONSTRAINT FK_T21_CENTER FOREIGN KEY (C21_CENTER_ID) REFERENCES t03_center(C03_CENTER_ID)
);
SELECT C21_POST_STATUS FROM t21_center_posts;
SELECT * FROM t21_center_posts WHERE C21_CENTER_ID IS NULL;

SELECT * FROM t21_center_posts;t11_system_notification
